...
def send_cmd(conn, command):
    ...
    1 second for the command to be processed. Sometimes this has to be increased, can be very tricky
    """
...
def main():
    ...
    # Start an interactive shell and collct the prompt
    conn = conn_params.invoke_shell()
    time.sleep(1.0)
    print(f"Logged into {get_output(conn).strip()} successfully")

    # Send the confirmation string to the device
